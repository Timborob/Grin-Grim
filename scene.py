...
class Scene (object):
    ...
    def _draw(self, dt):
        ...
        self.background_color = getattr( self, "background_color", Color(0,0,0,1) )
		
        if type(self.background_color) is str:
            self.background_color = webcolors.HexToColor( webcolors.CSS3_NAMES_TO_HEX[self.background_color] )
        
        background( self.background_color.r,self.background_color.g, self.background_color.b, self.background_color.a ) #Maybe add in some background colours here
        for k,v in enumerate( _SpriteNodes ):
            v.Draw()
...
class SpriteNode():
	def __init__( self, texture=None, position=(0,0), z_position= 0.0, scale=1.0, x_scale=1.0, y_scale=1.0, alpha=1.0, speed=1.0, parent=None, size=None, color='white', blend_mode=0 ):
		self.anchor_point = (0,0)
		self.blend_mode = blend_mode
		self.color = color
		self.shader = None #shader
		self.size = size
		self.texture = "plc:Character_Pink_Girl" #texture)
		self.position = position
		
		load_image("plc:Character_Pink_Girl")
		
		_SpriteNodes.append( self )
		
	def Draw( self ):
		if self.size == None:
			self.size = (0,0)
		self.size = getattr( self, "size", (0,0) )
			
		image( self.texture, self.position[0], self.position[1], self.size[0], self.size[1])
