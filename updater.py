...
def extract_git_id(git):
    print(git)
    m = re.match('^http(s?)://([\w-]*\.)?github\.com/(?P<user>[\w-]+)/(?P<repo>[\w-]*)((/tree|/blob)/(?P<branch>[\w-]*))?'), git)
    return m
...
def git_download(url):
    ...
    if m:
        ...
        try:
                ...
                f,headers=urllib.request.urlretrieve(u,reporthook=functools.partial(dlProgress,u))
                z=zipfile.ZipFile(f)
                z.extractall()
                print(z.namelist())
        except:
